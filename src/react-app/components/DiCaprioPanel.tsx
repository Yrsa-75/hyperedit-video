...
export default function DiCaprioPanel({
  ...
  const generateFromImage = async (videoPrompt: string, imageId: string) => {
    ...
    try {
      const response = await fetch(`${FFMPEG_SERVER_URL}/session/${sessionId}/generate-video`, {
        method: 'POST',
  ...
  const restyleVideo = async (stylePrompt: string, videoId: string) => {
    ...
    try {
      const response = await fetch(`${FFMPEG_SERVER_URL}/session/${sessionId}/restyle-video`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: stylePrompt,
  ...
  const removeBackground = async (videoId: string) => {
    ...
    try {
      const response = await fetch(`${FFMPEG_SERVER_URL}/session/${sessionId}/remove-video-bg`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          videoAssetId: videoId,
                    ...
                    {message.video && (
                      <div className="mt-3">
                        <div className="relative rounded-lg overflow-hidden bg-zinc-900">
                          <video
                            src={`${FFMPEG_SERVER_URL}${message.video.streamUrl}`}
                            controls
                            className="w-full h-auto"
                            poster={message.video.thumbnailUrl ? `${FFMPEG_SERVER_URL}${message.video.thumbnailUrl}` : undefined}
                          />
